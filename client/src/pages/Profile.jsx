  ...
  const load = async () => {
    try {
      ...
      setForm({
        ...
        dateOfBirth: meRes?.data?.dateOfBirth
          ? meRes.data.dateOfBirth.substring(0, 10)
          : '',
        bio: meRes?.data?.bio || '',
        avatarUrl: meRes?.data?.avatarUrl || '',
      });

      // ✅ Load stats separately (so it won’t break profile)
      try {
        const statsRes = await getMyStats();
        setStats({
          postsCount: statsRes?.data?.postsCount || 0,
          mediaCount: statsRes?.data?.mediaCount || 0,
          friendsCount: statsRes?.data?.friendsCount || 0,
        });
      } catch (err) {
